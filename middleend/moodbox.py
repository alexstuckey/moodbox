...
while cont == False:
    ...
    if personPresent == False:
        ...
        try:
            getJSONForMoodRequest = 'http://community.dur.ac.uk/mohammed.m.rahman/moodbox/backend/api.php?action=getTrack&emotion=' + str(
                playlist)
            getJSONForMoodData = urllib2.Request(getJSONForMoodRequest)
            response = urllib2.urlopen(getJSONForMoodData)
...
while True:
    try:
        ...
        getControlRequstData = urllib2.Request(getControlRequst)
        getControlRequstData = urllib2.urlopen(getControlRequst)
        getControlRequstData = getControlRequstData.read()
